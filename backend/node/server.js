...
// ---------------------------------------------------------------------------
// Google Drive Image Import & MongoDB GridFS File Serving
// ---------------------------------------------------------------------------
function extractDriveFileId(url) {
  ...
  if (/^[a-zA-Z0-9_-]{20,50}$/.test(cleanUrl)) return cleanUrl;
  return null;
}

app.post("/api/admin/import-drive-image", requireStaff, async (req, res) => {
  try {
    const { url } = req.body || {};
    const fileId = extractDriveFileId(url);
    if (!fileId) {
      return res.status(400).json({ detail: "Invalid Google Drive link format" });
    }
    ...
    let resolvedContentType = "image/jpeg";
    ...
    if (!imageBuffer) {
      return res.status(400).json({
        detail:
          "Unable to download image from Google Drive. Ensure the file sharing is set to 'Anyone with the link'.",
      });
    }

    if (imageBuffer.length > 15 * 1024 * 1024) {
      return res.status(413).json({ detail: "Image exceeds 15MB limit" });
    }

    const db = getDb();
    const bucket = new GridFSBucket(db);
    const uploadStream = bucket.openUploadStream(`drive-${fileId}`, {
      metadata: {
    ...
    return res.status(201).json({ path: `mongo/${uploadStream.id}` });
  } catch (err) {
    console.error("[Drive Import] Error:", err);
    return res.status(500).json({ detail: `Drive import failed: ${err.message}` });
  }
});
...
app.get("/api/files/*", async (req, res) => {
  try {
    ...
    if (filePath.startsWith("mongo/")) {
      const fileIdStr = filePath.replace(/^mongo\//, "").trim();
      let objId;
      try {
        objId = new ObjectId(fileIdStr);
      } catch {
      ...
      const db = getDb();
      const bucket = new GridFSBucket(db);
      const files = await bucket.find({ _id: objId }).limit(1).toArray();
    ...
    return res.status(500).json({ detail: "Failed to serve file" });
  }
});
